...
async function callAI(prompt: string, apiKey: string, system?: string): Promise<string> {
  if (!apiKey) return "";
...
export async function categorizeContent(
  ...
  if (!apiKey) return { title: "Untitled Sermon", category: "Faith", adjustedContent: text };

  const level = ageBracket === "junior"
    ? "ages 5-12 (simple vocabulary, short sentences)"
    : "ages 13-22 (appropriate for teens, nuanced concepts)";
...
Generate exactly 8 questions for ${level}. Ensure questions test comprehension of the text.`;
  ...
  const response = await callAI(prompt, apiKey, systemPrompt);
  try {
