...
export default function SeniorSermonArticleClient({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (found) {
          const qRes = await fetch(`/api/questions?age=senior&sermonIds=${found.id}`);
          ...
          <article className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
            <div className="relative aspect-video bg-gradient-to-br from-bible-600 via-bible-700 to-purple-800">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-8">
                ...
                <p className="mt-3 text-purple-100 text-lg max-w-2xl">{excerpt}</p>
              </div>
            </div>
