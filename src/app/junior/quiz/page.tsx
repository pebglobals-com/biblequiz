      ...
      <div className="max-w-lg w-full">
        <div className="glass rounded-3xl p-10 border border-emerald-500/20 text-center animate-scale-in">
          ...
          <p className="text-gray-400 mb-8">
            Challenge yourself with {sermons.length > 0 ? `${sermons.length} ` : ""}study topics from the Junior category.
          </p>
          <Link
            href={`/quiz/play?age=junior&ids=${sermons.length > 0 ? sermons.map(s => s.id).join(",") : "all"}`}
